...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Call Dicenode with fAggressive=true to
    // get as much data as possible.
    // Rewrite dicenoded data to wallet.dat
    ...
    std::vector<CDBEnv::KeyValPair> dicenodedData;
    bool allOK = dbenv.Dicenode(newFilename, true, dicenodedData);
    if (dicenodedData.empty()) {
        LogPrintf("Dicenode(aggressive) found no records in %s.\n", newFilename);
        return false;
    }
    LogPrintf("Dicenode(aggressive) found %u records\n", dicenodedData.size());
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, dicenodedData) {
        if (fOnlyKeys) {
